...
mod tests {
    ...
    #[test]
    fn shed_then_reload_then_snapshot_equals_unshed() {
        // The `from-kzip` snapshotter writes a snapshot, then drops
        // its in-memory accumulator to free RAM (`needs_reload`).
        // On the next snapshot it repopulates from the on-disk
        // partial before merging the next batch. Both paths must
        // produce byte-identical final indices.
        let shed_path   = tmp("shed-final");
        let unshed_path = tmp("unshed-final");
        let partial     = tmp("shed-partial");

        // Two batches of disjoint data simulating two snapshot
        // intervals' worth of CU output.
        let mk_batch_a = || {
            let mut b = IndexBuilder::new();
            let s = sym_of("kythe:c++:a##A.cpp#fnA");
            b.upsert_sym(s, kind::FUNCTION, lang::CXX, "kythe:c++:a##A.cpp#fnA");
            b.add_alias(s, "ns::fnA");
            b.upsert_file(1, "/aosp/A.cpp");
            b.add_xref(s, role::DECL, 1, 100);
            b.add_call(s, sym_of("kythe:c++:a##A.cpp#fnB"), role::CALL);
            b
        };
        let mk_batch_b = || {
            let mut b = IndexBuilder::new();
            let s = sym_of("kythe:c++:a##A.cpp#fnB");
            b.upsert_sym(s, kind::FUNCTION, lang::CXX, "kythe:c++:a##A.cpp#fnB");
            b.add_alias(s, "ns::fnB");
            b.upsert_file(2, "/aosp/B.cpp");
            b.add_xref(s, role::DEF, 2, 200);
            b
        };

        // Shed path: snapshot batch A, drop builder, reload from
        // partial, merge batch B, finalize.
        let mut shed = IndexBuilder::new();
        shed.merge_from(mk_batch_a());
        shed.snapshot(&partial).unwrap();
        shed = IndexBuilder::new();  // shed
        let ix = Index::open(&partial).unwrap();
        shed.populate_from_index(&ix).unwrap();
        drop(ix);
        shed.merge_from(mk_batch_b());
        shed.finish(&shed_path).unwrap();

        // Reference path: keep both batches in the same builder
        // throughout, no shed/reload.
        let mut unshed = IndexBuilder::new();
        unshed.merge_from(mk_batch_a());
        unshed.merge_from(mk_batch_b());
        unshed.finish(&unshed_path).unwrap();

        // Compare via the query surface — every alias, every xref,
        // every call must round-trip.
        let r1 = Index::open(&shed_path).unwrap();
        let r2 = Index::open(&unshed_path).unwrap();
        for name in ["kythe:c++:a##A.cpp#fnA", "kythe:c++:a##A.cpp#fnB",
                     "ns::fnA", "ns::fnB"] {
            assert_eq!(r1.sym_for_name(name), r2.sym_for_name(name),
                "name {name} resolves differently after shed");
        }
        let s_a = r1.sym_for_name("kythe:c++:a##A.cpp#fnA").unwrap();
        let s_b = r1.sym_for_name("kythe:c++:a##A.cpp#fnB").unwrap();
        let r1_xa: Vec<_> = r1.xrefs(s_a, 0, 255).collect();
        let r2_xa: Vec<_> = r2.xrefs(s_a, 0, 255).collect();
        assert_eq!(r1_xa, r2_xa, "fnA xrefs diverged after shed");
        let r1_xb: Vec<_> = r1.xrefs(s_b, 0, 255).collect();
        let r2_xb: Vec<_> = r2.xrefs(s_b, 0, 255).collect();
        assert_eq!(r1_xb, r2_xb, "fnB xrefs diverged after shed");
        let r1_calls: Vec<_> = r1.calls_from(s_a).into_iter().collect();
        let r2_calls: Vec<_> = r2.calls_from(s_a).into_iter().collect();
        assert_eq!(r1_calls, r2_calls, "fnA calls diverged after shed");

        let _ = std::fs::remove_file(&shed_path);
        let _ = std::fs::remove_file(&unshed_path);
